<script>
  ...
  // onDetected is called with (halfDuplex, pinSwap) on SUCCESS so the caller
  // can apply the result to FC.ESC_SENSOR_CONFIG - this component has no
  ...
  // went dirty and hunt for its own Save button. Mirrors the receiver tab's
  // RxWiringDetectWizard.svelte (see rotorflight-firmware's
  // docs/rx-wiring-autodetect-design.md for the underlying mechanism) - kept
  // as a separate component rather than a shared one since the wire shape
  // genuinely differs (no `inverted` field, 4 combos not 8).
  let { onDetected, onButtonDisabled, onClose, onSaveRequested } = $props();

  // Keep in sync with rotorflight-firmware's sensors/esc_sensor.h
  // escSensorTrialState_e.
  const ESC_SENSOR_TRIAL = {
    IDLE: 0,
    RUNNING: 1,
    SUCCESS: 2,
    FAILED: 3,
  ...
  // Small diagnostic line - see rotorflight-firmware's escSensorTrialStatus_t
  // for the extra fields this reads. Useful for any future bench-test report
  // without needing devtools open: whether the FC-side trial is actually
  // ticking (elapsedMs growing), seeing any frames at all (frameDelta), and
  // has the port open.
  ...
  function sendTrialQuery(action) {
    ...
    return new Promise((resolve) => {
      ...
      MSP.send_message(
        MSPCodes.MSP2_ESC_SENSOR_TRIAL,
        payload,
        false,
        finish,
        true,
  ...
  function onQueryFailed() {
    setWizard(
      ...
      "motorsEscWiringDetectUnsupported",
      100,
    );
  ...
  async function queryTrial(startProcedure = false) {
    ...
    // Bench-diagnostic fields - appended after the original 6-byte response,
    // so older firmware without them still parses fine (this block just
    // doesn't run).
    ...
    if (state === ESC_SENSOR_TRIAL.SUCCESS) {
      ...
      // caller's onDetected writes it into FC.ESC_SENSOR_CONFIG, which rides
      // that tab's existing dirty-diff/Save/Revert flow exactly like any
      ...
      // "Save and Reboot" discards the result (Telemetry.svelte's onClose
      // reverts FC.ESC_SENSOR_CONFIG since `saved` never got set), so an
      // unattended auto-close would silently throw away a successful
      // detection. The result sits here until the user takes an explicit
      // action.
      setWizard(
        "motorsEscWiringDetectWizardStep2",
  ...
  // scan abandoned mid-flight doesn't leave the ESC(s) sitting on a wiring
...
<dialog bind:this={dialogEl} onclose={handleDialogClose}>
  <h3>{$i18n.t("motorsEscWiringDetectWizardTitle")}</h3>
  ...
  <div class="wizard-actions">
    {#if canRetry}
      <button class="btn" onclick={onClickRetry}>
        {$i18n.t("motorsEscWiringDetectWizardRetry")}
      </button>
    {/if}
    <button class="btn" onclick={onClickClose}>
      {$i18n.t("motorsEscWiringDetectWizardClose")}
    </button>
    {#if canSave}
